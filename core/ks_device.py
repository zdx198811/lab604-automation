...
class M8195A:
    ...
    def __init__(self, addr, rm_path='@py', Glb_Tout = 10000):
        """ Initialization - connect to the device.
        
        rm_path: resource manager path. For windows, use Keysight official
        IO control lib; For linux, by default use '@py' which is a pure 
        python implementation from pyvisa.
        addr: address string, e.g. "TCPIP0::172.24.145.91::inst0::INSTR"
        Glb_Tout: global timeout; 
        """
        rm = visa.ResourceManager(rm_path)
        self.msgbf = MessageBuf(msg_src = 'M8195A')  # store messages and send message to GUI
        # Open Connection
        try:
